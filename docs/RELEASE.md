...
| v0.9.1 | `1526af2` | 2026-09-10T20:47Z | Harness collects the enterprise managed policy (`managed-settings.json` + `/etc/claude-code` default root) — #46/#47, reported by a blog-post reviewer, same-day fix-to-release. Smoke: packaged binary picked up a managed-settings fixture, permission broadening fired R49 `[HIGH]`, chain verified; install one-liner confirmed serving 0.9.1. Placeholder `LICENSE_SECRET`. |
